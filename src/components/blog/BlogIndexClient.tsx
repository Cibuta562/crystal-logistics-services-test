  ...
  const filtered = useMemo(() => {
    ...
    return posts.filter((p) => {
      const hitTxt =
          ...
          (p.tags ?? []).some((tag: string) => tag.toLowerCase().includes(txt));
      const hitTags =
          selected.length === 0 ||
          selected.every((tag: string) => (p.tags ?? []).includes(tag));
...
function Latest({
  ...
  return (
      <div className="border rounded-2xl p-4">
        <h3 className="text-xl font-semibold mb-3">{t("latestPosts")}</h3>
        <ul className="space-y-3">
          {latest.map((p) => (
              <li key={p.slug} className="flex gap-3">
                {p.coverUrl ? (
                    <div className="relative h-14 w-20 overflow-hidden rounded-md bg-gray-200">
                      <Image
                          src={p.coverUrl}
                          alt={p.title}
                          fill
